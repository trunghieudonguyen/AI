...
*/
void Graph::readGraphLT(const char* filename, const char * benfile)
{
   	FILE * pFile;
	...
	ben = vector<double>(numNodes+1);
	ben_ind = vector<UI>(numNodes+1);
...
*/
void Graph::readGraphIC(const char* filename, const char * benfile)
{
    	FILE * pFile;
    	pFile = fopen(filename, "rb");
    	fread(&numNodes, sizeof(int), 1, pFile);
    	...
    	node_deg=vector<int>(numNodes + 1);
	ben = vector<double>(numNodes + 1);
	ben_ind = vector<unsigned int>(numNodes+1);
    	fread(&node_deg[1], sizeof(int), numNodes, pFile);
...
double Graph::getTotalBenefit() const
{
	return totalBen;
}

double Graph::getTotalBenefit()
{
	return totalBen;
}

// return benenfit index
const std::vector<unsigned int> & Graph::getBenIndex() const
{
	return ben_ind;
}

const std::vector<double> & Graph::getBen() const
{
	return ben;
}

const std::vector<unsigned int> & Graph::getBenIndex()
{
	return ben_ind;
}


const std::vector<double> & Graph::getBen()
{
	return ben;
}                                               
...
unsigned int Graph::getNodeInBen(unsigned int ind){
	return ben_ind[ind];
}
...
HyperGraph::HyperGraph(unsigned int n, Graph & g)
{
	...
	numNodes = n;
	curEdge=0;
	vector<double> ben = g.getBen();
	benSize = ben.size();
	bDist = vector<UI>(benSize+1,0);
	for (unsigned int i = 1; i < benSize; ++i){
		ben[i]+=ben[i-1];
	}
	double tmp;
	for (unsigned int i = 1; i < benSize+1; ++i){
		tmp = g.UI_MAX*ben[i-1]/ben[benSize-1];
		if (tmp >= g.UI_MAX)
			bDist[i] = g.UI_MAX;
		else
                	bDist[i]=(UI)(tmp);
...
{	
	...
	bool t = false;
        ...
        unsigned int cur = g.getNodeInBen(randIndex_bin(bDist,benSize)-1);
...
void HyperGraph::pollingLT1(Graph &g, vector<bool> &visit, vector<int> &visit_mark)
{
        unsigned int i;
        unsigned int gSize = g.getSize();
	int tmp = randIndex_bin(bDist,benSize);
        unsigned int cur = g.getNodeInBen(tmp-1);
        unsigned int num_marked = 0;
        for (i = 0; i < gSize; ++i){
                if (visit[cur] == true) break;
                visit[cur] = true;
